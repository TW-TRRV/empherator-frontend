      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 w-full">
          {products.map((product) => {
            const images = JSON.parse(product.default_images || '{}');
            const primaryImage = images.primary || 'https://placehold.co/600x400?text=Placeholder';
            return (
              <div
                key={product.id}
              ...
              >
                {/* Product Image */}
                <Link href={`/product/${product.id}`} className="aspect-[4/3] w-full bg-obscure-darker overflow-hidden p-6 flex items-center justify-center">
                  <img
                    src={primaryImage}
                    alt={product.name}
